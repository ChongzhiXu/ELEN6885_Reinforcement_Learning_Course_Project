...
class Epsilon_Strategy:
    ...
    def __init__(self, alpha=0.5, gamma=0.2, epsilon=0.1, episodes_to_min_eps=100, min_eps=0.01):
        ...
        self.q_matrix = np.random.uniform(low=0, high=0, size=(3, 3))  # random matrix with size 3*3
    ...
    def Next_Move(self, ):
        ...
        if history_length <= 1:  # Play randomly in the first TWO rounds
            ...
            opponent_prev_move = self.RPS_list.index(self.opponent_history[-1])
        ...
        return self.RPS_list[self.action]
    ...
    def Learn(self, opponent_history, self_history):
        ...
        if history_length >= 2:  # Play randomly in the first TWO rounds
            ...
            state = self.RPS_list.index(self.opponent_history[-2])  # opponent previous, previous play index as state
            next_state = self.RPS_list.index(self.opponent_history[-1])  # opponent previous play index as next state
            action = self.RPS_list.index(self.self_history[-2])  # self previous play index as action
